...
try:
  ...
  from mujoco import mjtEq as MujocoEq                # type: ignore
  from mujoco import MjModel as MujocoModel           # type: ignore
  from mujoco import mjtObj as MujocoObject           # type: ignore
  from mujoco import mjtJoint as MujocoJoint          # type: ignore
  from mujoco import mj_name2id as mujoco_name2id     # type: ignore
  from mujoco import mj_forward as mujoco_forward     # type: ignore
  from mujoco import mj_jacSite as mujoco_jacSite     # type: ignore
except ImportError as e: 
  import gymnasium
  raise gymnasium.error.DependencyNotInstalled( f"{e}. (HINT: you need to install mujoco.)")
...
def reset_mocap_welds(model, data):
    ...
    if model.nmocap > 0 and model.eq_data is not None:
        for i in range(model.eq_data.shape[0]):
            if model.eq_type[i] == MujocoEq.mjEQ_WELD:
...
def get_site_jacp(model, data, site_id):
    ...
    mujoco_jacSite(model, data, jacp, None, site_id)
...
def get_site_jacr(model, data, site_id):
    ...
    mujoco_jacSite(model, data, None, jacr, site_id)
...
def set_joint_qpos(model, data, name, value):
    ...
    joint_id = mujoco_name2id(model, MujocoObject.mjOBJ_JOINT, name)
    joint_type = model.jnt_type[joint_id]
    joint_addr = model.jnt_qposadr[joint_id]

    if joint_type == MujocoJoint.mjJNT_FREE:
        ndim = 7
    elif joint_type == MujocoJoint.mjJNT_BALL:
        ndim = 4
    else:
        assert joint_type in (MujocoJoint.mjJNT_HINGE, MujocoJoint.mjJNT_SLIDE)
        ndim = 1
...
def get_site_xpos(model, data, name):
    site_id = mujoco_name2id(model, MujocoObject.mjOBJ_SITE, name)
...
def set_mocap_pos(model, data, name, value):
    body_id = mujoco_name2id(model, MujocoObject.mjOBJ_BODY, name)
    mocap_id = model.body_mocapid[body_id]
    data.mocap_pos[mocap_id] = value


def set_mocap_quat(model: MujocoModel, data: MujocoData, name: str, value):
    body_id = mujoco_name2id(model, MujocoObject.mjOBJ_BODY, name)
    mocap_id = model.body_mocapid[body_id]
    data.mocap_quat[mocap_id] = value


def get_site_xmat(model: MujocoModel, data: MujocoData, name: str):
    site_id = mujoco_name2id(model, MujocoObject.mjOBJ_SITE, name)
...
def extract_mj_names(model: MujocoModel, obj_type: MujocoObject) -> tuple[Union[tuple[str, ...], tuple[()]], dict[str, int], dict]:
  ...
  elif obj_type == MujocoObject.mjOBJ_SITE: name_addr = model.name_siteadr; n_obj = model.nsite
  elif obj_type == MujocoObject.mjOBJ_LIGHT: name_addr = model.name_lightadr; n_obj = model.nlight
  elif obj_type == MujocoObject.mjOBJ_CAMERA: name_addr = model.name_camadr; n_obj = model.ncam
  elif obj_type == MujocoObject.mjOBJ_ACTUATOR: name_addr = model.name_actuatoradr; n_obj = model.nu
  elif obj_type == MujocoObject.mjOBJ_SENSOR: name_addr = model.name_sensoradr; n_obj = model.nsensor
  elif obj_type == MujocoObject.mjOBJ_TENDON: name_addr = model.name_tendonadr; n_obj = model.ntendon
  elif obj_type == MujocoObject.mjOBJ_MESH: name_addr = model.name_meshadr; n_obj = model.nmesh
  else: raise ValueError(f"{obj_type} was passed as the MuJoCo model object type but can only be of the following MujocoObject enum types: {set_joint_qpos}.")
  id2name = {i: None for i in range(n_obj)}; name2id = {}
  for addr in name_addr:
    name = model.names[addr:].split(b"\x00")[0].decode()
    if name:
      obj_id = mujoco_name2id(model, obj_type, name)
      assert 0 <= obj_id < n_obj and id2name[obj_id] is None
      name2id[name] = obj_id; id2name[obj_id] = name
...
class MujocoModelNames:
  ...
  def __init__(self, model: MujocoModel):
    """Access MujocoObject object names and ids of the current MuJoCo model.
    Args: model: mjModel of the MuJoCo environment. """
    (self._body_names, self._body_name2id, self._body_id2name) = extract_mj_names(model, MujocoObject.mjOBJ_BODY)
    (self._joint_names, self._joint_name2id, self._joint_id2name) = extract_mj_names(model, MujocoObject.mjOBJ_JOINT)
    (self._geom_names, self._geom_name2id, self._geom_id2name) = extract_mj_names(model, MujocoObject.mjOBJ_GEOM)
    (self._site_names, self._site_name2id, self._site_id2name) = extract_mj_names(model, MujocoObject.mjOBJ_SITE)
    (self._camera_names, self._camera_name2id, self._camera_id2name) = extract_mj_names(model, MujocoObject.mjOBJ_CAMERA)
    (self._actuator_names, self._actuator_name2id, self._actuator_id2name) = extract_mj_names(model, MujocoObject.mjOBJ_ACTUATOR)
    (self._sensor_names, self._sensor_name2id, self._sensor_id2name) = extract_mj_names(model, MujocoObject.mjOBJ_SENSOR)

  @property
  def body_names(self): return self._body_names

  @property
  def body_name2id(self): return self._body_name2id

  @property
  def body_id2name(self): return self._body_id2name

  @property
  def joint_names(self): return self._joint_names

  @property
  def joint_name2id(self): return self._joint_name2id

  @property
  def joint_id2name(self): return self._joint_id2name

  @property
  def geom_names(self): return self._geom_names

  @property
  def geom_name2id(self): return self._geom_name2id

  @property
  def geom_id2name(self): return self._geom_id2name

  @property
  def site_names(self): return self._site_names

  @property
  def site_name2id(self): return self._site_name2id

  @property
  def site_id2name(self): return self._site_id2name

  @property
  def camera_names(self): return self._camera_names

  @property
  def camera_name2id(self): return self._camera_name2id

  @property
  def camera_id2name(self): return self._camera_id2name

  @property
  def actuator_names(self): return self._actuator_names

  @property
  def actuator_name2id(self): return self._actuator_name2id

  @property
  def actuator_id2name(self): return self._actuator_id2name

  @property
  def sensor_names(self): return self._sensor_names

  @property
  def sensor_name2id(self): return self._sensor_name2id

  @property
  def sensor_id2name(self): return self._sensor_id2name
